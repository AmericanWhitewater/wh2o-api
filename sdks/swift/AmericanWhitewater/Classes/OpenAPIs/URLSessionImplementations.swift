...
internal typealias AmericanWhitewaterAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, AmericanWhitewaterAPIChallengeHandler>()
...
internal class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    internal var taskDidReceiveChallenge: AmericanWhitewaterAPIChallengeHandler?
    ...
    @discardableResult
    override internal func execute(_ apiResponseQueue: DispatchQueue = AmericanWhitewaterAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    internal func buildHeaders() -> [String: String] {
        ...
        for (key, value) in AmericanWhitewaterAPI.customHeaders {
            httpHeaders[key] = value
        }
        return httpHeaders
    }
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.AmericanWhitewater.\(UUID().uuidString)")
                }
