...
open class ApiClient(val baseUrl: String, val client: OkHttpClient = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["X-WH2O-API-KEY"].isNullOrEmpty()) {
            if (apiKey["X-WH2O-API-KEY"] != null) {
                if (apiKeyPrefix["X-WH2O-API-KEY"] != null) {
                    requestConfig.headers["X-WH2O-API-KEY"] = apiKeyPrefix["X-WH2O-API-KEY"]!! + " " + apiKey["X-WH2O-API-KEY"]!!
                } else {
                    requestConfig.headers["X-WH2O-API-KEY"] = apiKey["X-WH2O-API-KEY"]!!
                }
            }
        }
    }
